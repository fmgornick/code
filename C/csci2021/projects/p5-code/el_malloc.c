...
int el_init(){
  void *heap = mmap(EL_HEAP_START_ADDRESS, EL_HEAP_INITIAL_SIZE, 
		PROT_WRITE | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
el_blockhead_t *el_block_below(el_blockhead_t *block){
	if (block == el_ctl.heap_start) return NULL;
...
el_blockhead_t *el_find_first_avail(size_t size){
	...
	while (ptr != el_ctl.avail->end) {
		if (ptr->size >= size + EL_BLOCK_OVERHEAD) return ptr;
		ptr = ptr->next;
	}
	return NULL;
}
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
	if (block->size < new_size + EL_BLOCK_OVERHEAD) return NULL;
	...
	el_blockfoot_t *lower_foot = el_get_footer(lower_head);
	lower_foot->size = new_size;

	el_blockhead_t *upper_head = el_block_above(lower_head);
	upper_head->size = old_size - (new_size + EL_BLOCK_OVERHEAD);
...
void *el_malloc(size_t nbytes){
	...
	if (user_block == NULL) return NULL;

	el_remove_block(el_ctl.avail, user_block);
	el_blockhead_t *remain_block = el_split_block(user_block, nbytes);

	user_block->state = EL_USED;
	el_add_block_front(el_ctl.used, user_block);

	if (remain_block != NULL) {
		remain_block->state = EL_AVAILABLE;
		el_add_block_front(el_ctl.avail, remain_block);
	}

	void *usr_ptr = PTR_PLUS_BYTES(user_block, sizeof(el_blockhead_t));
	return usr_ptr;
}
...
void el_merge_block_with_above(el_blockhead_t *lower){
	if (lower == NULL || lower->state != EL_AVAILABLE) return;

	el_blockhead_t *higher = el_block_above(lower);
	if (higher->next == NULL || higher->state != EL_AVAILABLE) return;
...
void el_free(void *ptr){
	el_blockhead_t *user_block = PTR_MINUS_BYTES(ptr, sizeof(el_blockhead_t));
	...
	el_blockhead_t *below = el_block_below(user_block);
