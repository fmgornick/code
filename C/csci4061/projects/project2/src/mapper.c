...
void writeIntermediateDS() {
  ...
  while (travNode != NULL) {
    ...
    while (tNode->next != NULL) {
      ret =
          write(fd, tNode->value, strlen(tNode->value)); // space after the word
      if (ret < 0) {
        printf("ERROR: Cannot write to file %s\n", wordFileName);
        exit(0);
      }
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
     ...
     * It is used to check the content in test.txt and the input file are the
     * same. Comment out the debug code when you are working on the final
     * submission. To satisify intermediate submission, your goal is to
     * implement sendChunkData() and getChunkData(). In addition, We Commented
     * out the code that is irrelevant to the intermediate submission. Once you
